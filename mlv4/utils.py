...
def initialize_agents(num_agents, embedding_loader, device, decay_rate, reinforcement_rate):
    ...
    for agent_id in range(num_agents):
        # Define cultural headwinds for each agent
        ...
        # Instantiate Agent with the device
        agent = Agent(agent_id, embedding_loader, cultural_headwinds, device, decay_rate, reinforcement_rate)
        agents.append(agent)
    return agents
# utils.py
...
def simulate_interactions(agents, num_interactions, vocabulary):
    for _ in tqdm(range(num_interactions), desc="Interactions"):
        ...
        print(agent_a, agent_b, message_words)
